...
pdf-kb-builder/
  ...
  references/
    dependencies.md
  assets/
    fonts/font-manifest.json       # 字体候选清单；不提交字体二进制
    windows-ocr/capabilities.json
    # 不提交 assets/rapidocr 或 ONNX 模型
  tools/                  # 安装脚本创建，放本技能自己的依赖；不提交
```

## 资产策略

仓库只保存技能说明、脚本、测试和轻量 manifest，不保存大体积运行资产：

- 不提交 `assets/rapidocr/` 或 RapidOCR ONNX 模型。
- 不提交字体二进制，例如 `NotoSansSC-VF.ttf`。
- RapidOCR 由 Python 包依赖提供；模型与运行缓存按 RapidOCR/onnxruntime 的运行环境处理。
- CJK 字体优先使用系统已有字体；缺少时，Windows 安装脚本只尝试安装系统字体 capability，不从技能包复制字体文件。
...
如果不想安装 Windows OCR/字体 capability，只安装本技能包可处理的 Python 依赖：
...
验证安装逻辑时，不要只在已有 `tools` 目录上重复跑。应先用 `-Uninstall` 删除本技能默认的 `tools`，再重新执行安装脚本；这能确认系统/全局 Python 包、`AI_TOOLS_HOME` 及其直接子目录和重新创建的技能 `tools` 会被联合检查，并且缺少的包才会补装。这个验证只允许删除本技能自己的 `tools`，不能删除 `AI_TOOLS_HOME` 根目录或其中已有工具。
