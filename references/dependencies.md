...
Windows 使用 `scripts/install_windows_dependencies.ps1` 安装依赖。脚本会先检测本机和目标目录中已有的 Python import；已有的包跳过，缺少的包才安装到指定目录，例如 `D:\ai_tools`。`pdf_kb.py` 会自动把 `AI_TOOLS_HOME` 加入 `sys.path`。
...
OCR capability 会按 Windows capability 状态检测；已安装则跳过。字体会先检测常见 CJK 字体文件；只在未找到字体文件时，才尝试安装 Windows CJK 字体 capability。
