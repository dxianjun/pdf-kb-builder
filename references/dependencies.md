...
验证安装流程时必须做一次空 `tools` 验证：先运行 `scripts/install_windows_dependencies.ps1 -Uninstall` 删除本技能默认 `tools`，再运行安装脚本。这样才能确认依赖检查会联合使用系统/全局 Python site-packages（`site.getsitepackages()`、`site.getusersitepackages()`、`sys.path`）、`AI_TOOLS_HOME`、`AI_TOOLS_HOME` 和重新创建的技能 `tools`，避免因为旧 `tools` 已经完整而误判验证通过。
